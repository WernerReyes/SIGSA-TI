...
import {
    DepartmentAllowed,
    type Department,
} from '@/interfaces/department.interace';
...
export const useApp = () => {
    ...
    const users = computed(
        () => (page.props as unknown as { users: User[] })?.users || [],
    );
    const availableAssets = computed(
        () =>
            (page.props as unknown as { availableAssets: Asset[] })
                ?.availableAssets || [],
    );
    const TIUsers = computed(
        () => (page.props as unknown as { TIUsers: User[] })?.TIUsers || [],
    );
    const departments = computed(
        () =>
            (page.props as unknown as { departments: Department[] })
                ?.departments || [],
    );
    const assetTypes = computed(
        () => (page.props as unknown as { types: AssetType[] })?.types || [],
    );
    const assetAccessories = computed(
        () =>
            (page.props as unknown as { accessories: Asset[] })?.accessories ||
            [],
    );
    const brands = computed(
        () => (page.props as unknown as { brands: Brand[] })?.brands || [],
    );
    const models = computed(
        () => (page.props as unknown as { models: AssetModel[] })?.models || [],
    );

    const slaPolicies = computed(
        () =>
            (page.props as unknown as { slaPolicies: SlaPolicy[] })
                ?.slaPolicies || [],
    );

    const echo = useEchoModel('App.Models.User', userAuth.value.staff_id);
    ...
    const isTIAssistantManager = computed(() => {
        return (
            isFromTI.value &&
            userAuth.value?.id_cargo === UserCharge.IT_ASSISTANT_MANAGER
        );
    });

    const isTIManager = computed(() => {
        return (
            isFromTI.value && userAuth.value?.id_cargo === UserCharge.IT_MANAGER
        );
    });
